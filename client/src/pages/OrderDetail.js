...
import { selectOrderById, fetchCustomerOrders, selectFetchCustomerOrdersStatus } from '../features/orders/ordersSlice';
  ...
  useEffect(() => {
    if (!state?.orderNr || fetchStatus !== 'succeeded') {
      ...
      const totalPrice = Object.keys(order).reduce(
        (acc, keyName) => acc + parseFloat(order[keyName].price) * parseInt(order[keyName].quantity, 10),
        0
      );

      const quantity = Object.keys(order).reduce((acc, keyName) => acc + parseInt(order[keyName].quantity, 10), 0);
            ...
            <div className="border-t-2">
              ...
              {Object.keys(order).map(productId => (
                <Card key={id} className="flex my-4 p-2 md:flex-row flex-col">
                  ...
                  />
                  <CardBody>
                    <h1 className="font-semibold text-gray-600">{order[productId].name}</h1>
                    <p className="mb-2">$ {parseFloat(order[productId].price) * parseInt(order[productId].quantity, 10)}</p>
                    <p className="text-gray-600 dark:text-gray-400">{products[order[productId].product_id].description}</p>
                    <p className="mt-2">Quantity: {order[productId].quantity}</p>
                  </CardBody>
                </Card>
              ))}
            </div>
