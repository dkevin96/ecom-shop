...
const createOrderDb = async ({ user_id, status, amount }) => {
  const text = `INSERT INTO orders (user_id, status, order_price) VALUES($1, $2, $3) RETURNING *`;
  const values = [user_id, status, amount];
...
const createProductInOrderDb = async ({
  order_id,
  product_id,
  quantity,
  price,
