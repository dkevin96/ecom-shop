...
const { products, auth, users, carts, payment, orders } = require('../controllers');
...
router
  ...
  .get('/auth/google/redirect', passport.authenticate('google', { session: false }), auth.loginGoogle)
  ...
  .get('/auth/facebook/redirect', passport.authenticate('facebook', { session: false }), auth.loginFacebook)
  // PRODUCTS
  ...
  .delete('/users/:id', passport.authenticate('jwt-admin', { session: false }), users.deleteUser) //Delete user and associated cart
  ...
  .post('/carts/self', passport.authenticate('jwt-customer', { session: false }), carts.syncCartSelf)

  //Adds a new product to user's cart
  .post('/carts/self/product', passport.authenticate('jwt-customer', { session: false }), carts.postProductInCartSelf)

  //Changes quantity of a product in user's cart
  .put('/carts/self/product', passport.authenticate('jwt-customer', { session: false }), carts.putCartSelf)

  //Deletes a product from user's cart
  .delete('/carts/self/product', passport.authenticate('jwt-customer', { session: false }), carts.deleteCartProductSelf)

  //Checks out a user's cart and places an order
  .post('/carts/self/checkout', passport.authenticate('jwt-customer', { session: false }), carts.checkoutCart)
  ...
  .get('/orders/self', passport.authenticate('jwt-customer', { session: false }), orders.getOrdersSelf)

  //Gets one order
  .get('/orders/review/:orderId', passport.authenticate('jwt-admin', { session: false }), orders.getOrderById);
